...
; for each pair a, b in the range c-k, I will add a list element (-a, -b) to require that at least one of them be negated


(defun add-negated-pairs (n c i k)
	(cond
		((> i k) nil)
		((equal c i) (add-negated-pairs n c (+ i 1) k))
		(t (cons (list (* -1 (node2var n c k)) (* -1 (node2var n i k))) (add-negated-pairs n c (+ i 1) k)))
 	)
)
...
(defun at-most-one-color (n c k)
	(cond
		((>= c k) nil)
		(t (append (add-negated-pairs n c c k) (at-most-one-color n (+ 1 c) k)))
	)
)
...
;
(defun rec-gen-edge-clauses (e c k)
	(cond 
		((> c k) nil)
		(t (cons 
			(list (* -1 (node2var (car e) c k)) (* -1 (node2var (car (cdr e)) c k)))
			(rec-gen-edge-clauses e (+ c 1) k)
		))
	)
)

(defun generate-edge-clauses (e k)
	(rec-gen-edge-clauses e 1 k)
  )
